...
REAL_FOODS = {
    ...
    "Pizza":                 ("🍕", 250),
    "Evening Out":           ("🌙", 400),
    "30 min YouTube without guilt": ("📺", 15),
    "10 min News":                  ("📰", 5),
}
...
# LEDGER 
# ============================================================
def log_transaction(action: str, coins: float, stars: float = 0):
    d["balance"]     = round(d["balance"] + coins, 2)
    d["super_stars"] += stars
...
def log_daily_breakdown(date_str: str, breakdown: list, net: float, stars: float, balance_after: float):
...
### Habits — Morning Brush +2 · Evening Brush +2 · Keep Alarm +2 · Make Plan +2 · Silence +4
...
with st.form(f"daily_log_form_{st.session_state.form_key}"):
    ...
    with hc1:
        ...
        alarm_5am = st.checkbox("⏰ Kept Alarm for Tomorrow 5:00 AM (+2 pts)")
    with hc2:
...
if submit_log:
    if today in d["daily_logs"]:
        ...
        d["balance"]      = round(d["balance"] - old["net_points"], 2)
    ...
    if alarm_5am: breakdown.append({"category":"⏰ Kept 5 AM Alarm","earned":2,"penalty":0})
    ...
    # ── 15% P.A. Interest Engine ──
    last_date = datetime.strptime(d["last_login"],"%Y-%m-%d").date()
    days_passed = (get_now().date() - last_date).days
    if days_passed > 0:
        if d["balance"] > 0:
            interest = round(d["balance"] * ((0.15 / 365) * days_passed), 2)
            d["balance"] = round(d["balance"] + interest, 2)
            d["history"].insert(0, {
                "date": today,
                "time": get_now().strftime("%H:%M"),
                "category": f"📈 15% p.a. Daily Interest Accrued ({days_passed} day(s))",
                "earned": interest, "penalty": 0, "net": interest, "stars": 0,
                "balance": d["balance"], "type": "transaction"
            })
        if days_passed == 1: d["streak"] += 1
        else:                     d["streak"]  = 1
    d["last_login"] = today

    d["balance"]     = round(d["balance"] + net_points, 2)
...
tab_food, tab_virtual, tab_star, tab_inv, tab_ach = st.tabs([
    "🍜  Nepali Foods & Ent.", "🏺  Virtual Artifacts", "⭐  Super Star Exchange", "🎒  My Collection", "🏆  Achievements"
])
...
with tab_star:
    st.info(f"Available Mastery Tokens: ⭐ **{round(d['super_stars'], 2)} Super Stars**")
    cols = st.columns(3)
    for idx, (item, (emoji, star_cost)) in enumerate(STAR_REWARDS.items()):
        with cols[idx % 3]:
            st.markdown(f"""
            <div class="shop-item">
                <span class="shop-item-emoji">{emoji}</span>
                <span class="shop-item-name">{item}</span>
                <span class="shop-item-price">⭐ {star_cost} Star(s)</span>
            </div>""", unsafe_allow_html=True)
            owned_count = d["inventory"].count(f"STAR::{item}::{emoji}")
            if owned_count > 0:
                st.markdown(f"""
                <div style="text-align:center; margin-bottom:0.3rem;">
                    <span style="background:#c9a84c; color:#fff; font-family:'Cinzel',serif;
                    font-size:0.65rem; padding:0.2rem 0.7rem; border-radius:10px;
                    letter-spacing:0.1em;">✓ ×{owned_count} in stash</span>
                </div>""", unsafe_allow_html=True)
            if st.button("Exchange Tokens", key=f"star_btn_{item}"):
                if d["super_stars"] >= star_cost:
                    d["super_stars"] -= star_cost
                    d["inventory"].append(f"STAR::{item}::{emoji}")
                    d["history"].insert(0, {
                        "date":    get_now().strftime("%Y-%m-%d"),
                        "time":    get_now().strftime("%H:%M"),
                        "category": f"⭐ Exchanged Stars for {item}",
                        "earned":  0, "penalty": 0, "net": 0, "stars": -star_cost,
                        "balance": d["balance"], "type": "transaction",
                    })
                    save_state()
                    st.toast(f"✦ Successfully claimed {item}! {emoji}", icon="⭐")
                    st.rerun()
                else:
                    st.error(f"❌ Need {star_cost - d['super_stars']} more Super Stars!")
...
with tab_inv:
    if not d["inventory"]:
        ...
        star_items   = [i for i in d["inventory"] if i.startswith("STAR::")]
        virtual_items = [i for i in d["inventory"] if not i.startswith("FOOD::") and not i.startswith("STAR::")]
                ...
                with col2:
                    if st.button(f"Eat 🍽️", key=f"eat_{raw_item}"):
                        ...
                        d["history"].insert(0, {
                            ...
                            "earned":   0, "penalty": 0, "net": 0, "stars": 0,
                            "balance":  d["balance"], "type": "transaction",
                        })
                        save_state(); check_achievements()
        ...
        if star_items:
            st.markdown("<div class='form-section-head'>🎬 Star Rewards — Ready to Redeem</div>", unsafe_allow_html=True)
            star_counts = Counter(star_items)
            for raw_item, count in star_counts.items():
                parts = raw_item.split("::")
                name  = parts[1] if len(parts) > 1 else raw_item
                emoji = parts[2] if len(parts) > 2 else "⭐"
                col1, col2 = st.columns([4, 1])
                with col1:
                    st.markdown(f'<div class="inv-item"><span>{emoji} {name}</span><span class="inv-count">×{count}</span></div>', unsafe_allow_html=True)
                with col2:
                    if st.button(f"Redeem 🎬", key=f"redeem_{raw_item}"):
                        d["inventory"].remove(raw_item)
                        d["history"].insert(0, {
                            "date":     get_now().strftime("%Y-%m-%d"),
                            "time":     get_now().strftime("%H:%M"),
                            "category": f"🎬 Redeemed Star Reward: {name}",
                            "earned":   0, "penalty": 0, "net": 0, "stars": 0,
                            "balance":  d["balance"], "type": "transaction",
                        })
                        save_state()
                        st.toast(f"✦ Redeemed {name}! Enjoy without guilt! {emoji}")
                        st.rerun()
...
with st.expander("⚙️  SECURE ADMINISTRATION — Vault Master Controls"):
    ...
    if unlock_pw == d["admin_password"]:
        ...
        old_change_pw = st.text_input("Confirm Old Password", type="password", key="old_change_pw")
        new_pw = st.text_input("New Password", type="password", key="new_pw")
        if st.button("🔑 Update Password"):
            if old_change_pw == d["admin_password"]:
                if new_pw: 
                ...
                st.error("Incorrect old password configuration.")
